/**
 * Daftari Plus — backend v2 (zero external dependencies)
 * -----------------------------------------------------
 ...
 *
 * Roles: 'admin' or 'saler'
 *   - admin: full access — manage products/stock (buy price, sell price,
 *     quantity), see profit margins, see all reports, record sales,
 *     add expenses/debts.
 *   - saler: record sales, add expenses, add debts, view sales — but
 *     never sees buy price or profit figures.
 */
...
function getUserByToken(token) {
  ...
  return db.prepare('SELECT id, full_name, business_name, email, role, created_at FROM users WHERE id = ?').get(session.user_id);
}
...
async function handleRegister(req, res) {
  ...
  const result = db.prepare(
    'INSERT INTO users (full_name, business_name, email, password_hash, role) VALUES (?, ?, ?, ?, ?)'
  ).run(fullName, businessName || null, email, passwordHash, role);

  const userId = Number(result.lastInsertRowid);
  const token = createSession(userId);
  const user = db.prepare('SELECT id, full_name, business_name, email, role, created_at FROM users WHERE id = ?').get(userId);
...
async function handleLogin(req, res) {
  ...
  sendJSON(res, 200, {
    token,
    user: { id: user.id, full_name: user.full_name, business_name: user.business_name, email: user.email, role: user.role, created_at: user.created_at }
  });
}
...
async function handleProductsPost(req, res) {
  ...
  const result = db.prepare(
    'INSERT INTO products (name, buy_price, sell_price, quantity, created_by) VALUES (?, ?, ?, ?, ?)'
  ).run(name, buyPrice, sellPrice, quantity, user.id);
...
async function handleSalesPost(req, res) {
  ...
  const result = db.prepare(
    `INSERT INTO sales (product_id, product_name, quantity, sell_price, buy_price, total, sold_by, sold_by_name)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(productId, product.name, quantity, product.sell_price, product.buy_price, total, user.id, user.full_name);
...
function handleExpensesGet(req, res) {
  const user = requireAuth(req, res);
  if (!user) return;
  const rows = db.prepare('SELECT * FROM expenses ORDER BY created_at DESC LIMIT 200').all();
...
async function handleExpensesPost(req, res) {
  ...
  const result = db.prepare(
    'INSERT INTO expenses (description, amount, created_by, created_by_name) VALUES (?, ?, ?, ?)'
  ).run(description, amount, user.id, user.full_name);
...
function handleDebtsGet(req, res) {
  const user = requireAuth(req, res);
  if (!user) return;
  const rows = db.prepare('SELECT * FROM debts ORDER BY created_at DESC LIMIT 200').all();
...
async function handleDebtsPost(req, res) {
  ...
  const result = db.prepare(
    'INSERT INTO debts (person_name, description, amount, created_by, created_by_name) VALUES (?, ?, ?, ?, ?)'
  ).run(personName, description || null, amount, user.id, user.full_name);
...
function handleDashboard(req, res) {
  ...
  const salesAgg = db.prepare('SELECT COUNT(*) as count, COALESCE(SUM(total),0) as revenue FROM sales').get();
  const expensesAgg = db.prepare('SELECT COALESCE(SUM(amount),0) as total FROM expenses').get();
  const debtsAgg = db.prepare("SELECT COALESCE(SUM(amount),0) as total FROM debts WHERE status = 'unpaid'").get();
  const productCount = db.prepare('SELECT COUNT(*) as count FROM products').get();
  const lowStock = db.prepare('SELECT id, name, quantity FROM products WHERE quantity <= 5 ORDER BY quantity ASC LIMIT 10').all();
  ...
  if (user.role === 'admin') {
    const costAgg = db.prepare('SELECT COALESCE(SUM(buy_price * quantity),0) as cost FROM sales').get();
