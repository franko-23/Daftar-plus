...
async function api(
  ...
  if(loanRepayReq && m === 'POST'){
    ...
    const rr = db.prepare(`
      ...
      VALUES(?,?,?,?,?,?,'PENDING')
    `).run(loan.id, u.business_id, phone, loan.total_due_tzs, 'palmpesa', tx);
  ...
  if(p === '/api/loans/repay/check-status' && m === 'POST'){
    ...
    if(pay.status === 'SUCCESSFUL'){
      return json(res, 200, {
        status: 'SUCCESSFUL',
        loan: db.prepare('SELECT * FROM loan_applications WHERE id=?').get(pay.loan_id)
      });
    }
    ...
    try{
      ...
      if(st === 'COMPLETED' || st === 'SUCCESSFUL'){
        ...
        const loan = markLoanRepaid(pay.loan_id);

        audit(u.business_id, u.id, 'LOAN_REPAID', `Order ${orderId}; TSh ${pay.amount_tzs}`);
  ...
  if(p === '/api/super/loans' && m === 'GET'){
    ...
    return json(res, 200, { loans: rows });
