...
function chatBotReply(text){
  ...
  for(const r of rules){
    if(r.k.some(word => t.includes(word))){
      return { text:r.a, needsHuman: !!r.forceHuman };
    }
  }
...
async function api(
  ...
  if(
    ...
    // Bot inajibu MOJA KWA MOJA tu ikiwa bado iko katika hali ya 'bot'.
    // Mara inapohitaji msaada wa binadamu (needs_human) au admin
    // ameshaingilia (with_admin), bot inazimika kiotomatiki na
    // inasubiri mfanyakazi hapa hapa — haijibu tena mpaka admin
    // amjibu (with_admin) au aimwage tena kwa bot ('Washa Bot Tena').
    if(convo.status === 'bot'){
