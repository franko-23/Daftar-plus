...
/* =========================================================
   ...
   iliyoidhinishwa — kama bado, iache tupu na Tegasms
   itatumia namba ya default).
   ========================================================= */
...
async function sendSms(to, message){
  ...
  try{
    ...
    const payload = { to: dest, message, type:'otp' };
    if(TEGASMS_SENDER_ID) payload.sender_id = TEGASMS_SENDER_ID;

    const url = TEGASMS_BASE_URL + '/send';
    ...
    if(!resp.ok){
      console.error('[otp] Tegasms imekataa ujumbe:', resp.status, 'URL:', url, 'BASE:', TEGASMS_BASE_URL, data);
...
http
  ...
  .listen(
    ...
    ()=>{
      ...
      console.log(
        smsConfigured()
          ? `[otp] SendAfrica imewekwa — BASE_URL: ${SENDAFRICA_BASE_URL}`
          : '[otp] SENDAFRICA_API_KEY HAIJAWEKWA — SMS hazitatumwa'
      );
