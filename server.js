...
async function api(
    ...
    if(existing){
      ...
      db.prepare(`
        ...
        SET chairman_name=?, chairman_phone=?, id_type=?, id_number=?, address=?,
            chairman_letter_file=?, id_photo_file=?, status='pending',
            review_note=NULL, reviewed_by=NULL, reviewed_at=NULL, updated_at=datetime('now')
        WHERE business_id=?
      `).run(chairmanName, chairmanPhone, idType, idNumber, address, letterFile, idFile, u.business_id);
      ...
      db.prepare(`
        ...
        (business_id, chairman_name, chairman_phone, id_type, id_number, address,
         chairman_letter_file, id_photo_file, status)
        VALUES(?,?,?,?,?,?,?,?,'pending')
      `).run(u.business_id, chairmanName, chairmanPhone, idType, idNumber, address, letterFile, idFile);

    }

    audit(u.business_id, u.id, 'LOAN_KYC_SUBMITTED', null);
  ...
  const kycFileReq = p.match(/^\/api\/loans\/kyc\/file\/(letter|id)$/);
  ...
  if(kycFileReq && m === 'GET'){
    ...
    const qBid = Number(q(req).business_id || 0);
    ...
    const fname = kycFileReq[1] === 'letter' ? k.chairman_letter_file : k.id_photo_file;
  ...
  if(p === '/api/super/loans/kyc' && m === 'GET'){
    ...
    const status = clean(q(req).status) || 'pending';
  ...
  if(p === '/api/super/loans' && m === 'GET'){
    ...
    const status = clean(q(req).status);
