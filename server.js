...
function chatBotReply(text){
  ...
  const rules = [
    {
      k: ['bei','gharama','malipo','kulipa','lipa','tsh','pesa ngapi'],
      a: 'Bei za Daftari+ zinaonekana ukurasa wa "Mipango na Bei" kwenye tovuti hii, na pia ndani ya app baada ya kujisajili. Malipo yanafanyika kwa urahisi kupitia simu (PalmPesa). Unataka nikuunganishe na mfanyakazi kwa maelezo zaidi ya bei?'
    },
    {
      k: ['jinsi ya kuanza','kuanzisha','kujisajili','register','fungua akaunti','anza'],
      a: 'Kuanza ni rahisi: bonyeza "Jisajili" juu ya ukurasa, jaza jina la biashara yako na namba ya simu, kisha unaweza kuanza kurekodi mauzo ndani ya dakika chache.'
    },
    {
      k: ['arifa','notification','push'],
      a: 'Daftari+ inaweza kukutumia arifa za mauzo na faida moja kwa moja kwenye simu yako kila baada ya muda unaochagua (mfano kila masaa 2). Hii inawashwa ndani ya "Mipangilio" baada ya kuingia.'
    },
    {
      k: ['saa ngapi','muda gani','fungua saa','kazi saa'],
      a: 'Timu yetu ya msaada inapatikana muda mwingi wa siku. Kama sasa hivi hakuna mtu mtandaoni, tutakujibu haraka iwezekanavyo — tafadhali subiri.'
    },
    {
      k: ['offline','bila mtandao','network','intaneti'],
      a: 'Daftari+ inaweza kuendelea kufanya kazi hata ukiwa na mtandao hafifu, na taarifa zinasawazishwa (sync) mtandao ukirudi.'
    },
    {
      k: ['wafanyakazi','saler','muuzaji','mfanyakazi'],
      a: 'Unaweza kuongeza wafanyakazi (saler) wasio na kikomo kwenye akaunti yako, kila mmoja akiwa na akaunti yake ya kuingia na ruhusa zake.'
    },
    {
      k: ['refund','kurudisha pesa','sikuridhika'],
      a: 'Pole kwa usumbufu. Tafadhali eleza tatizo lako kwa ufupi, na mfanyakazi wetu atakusaidia haraka kuhusu suala la malipo.'
    }
  ];

  for(const r of rules){
    if(r.k.some(word => t.includes(word))){
      return { text:r.a, needsHuman:false };
    }
  }

  return {
    text: 'Asante kwa ujumbe wako. Sijapata jibu la moja kwa moja kwa hilo, lakini nimemjulisha mfanyakazi wetu — atakujibu hivi karibuni hapa hapa kwenye mazungumzo haya.',
    needsHuman: true
  };
...
async function api(
  ...
  if(
    ...
    // Ikiwa mfanyakazi (admin) tayari yuko kwenye mazungumzo haya,
    // bot haijibu tena moja kwa moja — inasubiri mfanyakazi.
    if(convo.status !== 'with_admin'){
