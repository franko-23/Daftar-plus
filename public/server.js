 ...
 *  - subscription ikiwa haijalipiwa: ukumbusho mmoja kwa siku
 */
async function runPeriodicBusinessSummaries(){
  ...
  for(const biz of businesses){
    ...
    const dueMs = (sub ? intervalMin : Math.max(intervalMin, 1440)) * 60 * 1000;
    ...
    if(result.error){
      /* Jaribu tena baada ya dakika 10 badala ya kila dakika */
      pushRetryAfter.set(biz.id, Date.now() + 10 * 60 * 1000);
      continue;
    }
...
async function api(
  ...
  /* =======================================================
     BILLS — hakiki (public, bila login) — kwa QR code
     ======================================================= */

  {
    const mv = p.match(/^\/api\/bills\/verify\/([A-Za-z0-9]{6,20})$/);

    if(mv && m === 'GET'){

      if(!billVerifyAllowed(req)){
        return json(res,429,{ valid:false, error:'Maombi mengi sana. Jaribu tena baada ya dakika moja.' });
      }

      const row = db.prepare('SELECT * FROM bills WHERE code=?').get(mv[1].toUpperCase());

      if(!row){
        return json(res,404,{ valid:false, error:'Bili hii haijapatikana.' });
      }

      return json(res,200,{ valid:true, bill:billPublic(row) });
    }
  }


  /* =======================================================
     BILLS — tengeneza bili
     ======================================================= */
  ...
  if(
    ...
    if(!rawItems.length || rawItems.length > 50){
      return json(res,400,{ error:'Weka bidhaa angalau moja (kiwango cha juu 50).' });
    }

    // unganisha bidhaa zinazojirudia
    const want = new Map();
    for(const it of rawItems){
      const pid = Number(it.productId);
      const qty = Math.floor(Number(it.quantity));
      if(!Number.isInteger(pid) || !Number.isInteger(qty) || qty <= 0){
        return json(res,400,{ error:'Bidhaa au idadi si sahihi.' });
      }
    ...
    for(const [pid, qty] of want){
      ...
      if(!pdt){
        return json(res,400,{ error:'Bidhaa haipo kwenye biashara hii.' });
      }

      if(pdt.quantity < qty){
        return json(res,400,{ error:`Stoki ya "${pdt.name}" haitoshi (zipo ${pdt.quantity}).` });
      }

      const line = pdt.sell_price * qty;
      total += line;
      items.push({ name:pdt.name, qty, price:pdt.sell_price, line });
    }

    const biz = db.prepare(
      'SELECT name, lipa_namba, lipa_jina FROM businesses WHERE id=?'
    ).get(u.business_id);

    const lipa = clean(x.lipaNamba || biz.lipa_namba).replace(/[\s-]/g,'');

    if(!/^\d{4,15}$/.test(lipa)){
      return json(res,400,{ error:'Weka Lipa Namba sahihi (tarakimu tu).' });
    }

    const lipaJina = clean(x.lipaJina || biz.lipa_jina).slice(0,60);
    const custName = clean(x.customerName).slice(0,80);
    const code = newBillCode();

    const r = db.prepare(`
      INSERT INTO bills
      (
        code, business_id, shop_name, customer_name, items_json,
        total, lipa_namba, lipa_jina, created_by, created_by_name
      )
      VALUES(?,?,?,?,?,?,?,?,?,?)
    `).run(
      code, u.business_id, biz.name, custName || null,
      JSON.stringify(items), total, lipa, lipaJina || null,
      u.id, u.full_name
    );

    audit(u.business_id, u.id, 'BILL_CREATED', `Bill ${code} — TZS ${Math.round(total)}`);

    const row = db.prepare('SELECT * FROM bills WHERE id=?').get(Number(r.lastInsertRowid));

    return json(res,201,{
      bill: billPublic(row),
      verifyUrl: billVerifyUrl(code)
    });
  }


  /* =======================================================
     BILLS — tuma kwa SMS (Tegasms) — muuzaji ndiye anabonyeza
     ======================================================= */

  {
    const ms = p.match(/^\/api\/bills\/([A-Za-z0-9]{6,20})\/sms$/);
    ...
    if(ms && m === 'POST'){
      ...
      if(!validPhone(phone)){
        return json(res,400,{ error:'Namba ya simu si sahihi (mfano 07XXXXXXXX).' });
      }

      const row = db.prepare(
        'SELECT * FROM bills WHERE code=? AND business_id=?'
      ).get(ms[1].toUpperCase(), u.business_id);
      ...
      if(row.sms_count >= 3){
        return json(res,429,{ error:'Bili hii imeshatumwa mara 3 tayari.' });
      }

      if(!smsConfigured()){
        return json(res,503,{ error:'SMS haijawekwa kwenye server (TEGASMS_API_KEY).' });
      }

      const sms = await sendSms(phone, billSmsText(row));

      if(!sms.ok){
        return json(res,502,{ error:'SMS imeshindikana kutumwa. Jaribu tena.' });
      }

      db.prepare(`
        UPDATE bills
        SET sms_count=sms_count+1, sms_last_to=?, sms_last_at=datetime('now')
        WHERE id=?
      `).run(phoneE164(phone), row.id);

      audit(u.business_id, u.id, 'BILL_SMS_SENT', `Bill ${row.code} → ${phoneE164(phone)}`);

      return json(res,200,{ ok:true, sent:true, count:row.sms_count+1 });
    }
  }
  ...
  if(
    ...
    if(Object.prototype.hasOwnProperty.call(x,'lipa_namba')){
      const ln = String(x.lipa_namba||'').replace(/[\s-]/g,'');
      if(ln && !/^\d{4,15}$/.test(ln)){
        return json(res,400,{ error:'Lipa Namba lazima iwe tarakimu tu.' });
      }
      db.prepare('UPDATE businesses SET lipa_namba=?, lipa_jina=? WHERE id=?')
        .run(ln || null, String(x.lipa_jina||'').trim().slice(0,60) || null, u.business_id);
    }
...
/* =========================================================
   BILI (BILLS) — bili ya kutuma kwa mteja kwa SMS + QR ya kuhakiki
   ========================================================= */

try{ db.exec("ALTER TABLE businesses ADD COLUMN lipa_namba TEXT"); }catch{}
try{ db.exec("ALTER TABLE businesses ADD COLUMN lipa_jina TEXT"); }catch{}
...
CREATE INDEX IF NOT EXISTS idx_bills_business ON bills(business_id, created_at);
`);

const BILL_CODE_CHARS = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';

function newBillCode(){
  for(let t=0;t<10;t++){
    let c='';
    for(let i=0;i<10;i++) c += BILL_CODE_CHARS[crypto.randomInt(BILL_CODE_CHARS.length)];
    if(!db.prepare('SELECT 1 FROM bills WHERE code=?').get(c)) return c;
  }
  throw new Error('Imeshindwa kutengeneza code ya bili.');
}

function billVerifyUrl(code){
  return oauthBaseUrl() + '/bill.html?c=' + encodeURIComponent(code);
}
...
function billPublic(row){
  ...
  return {
    ...
    seller: row.created_by_name,
    status: row.status,
    created_at: row.created_at
  };
}

function billSmsText(row){
  const bill = billPublic(row);
  const n = v => Math.round(v).toLocaleString('en-US');
  const lines = bill.items.slice(0,6).map(i => `${i.name} x${i.qty} = ${n(i.line)}`);
...
/* kikomo rahisi cha maombi ya public verify (kwa IP) */
const billVerifyHits = new Map();
function billVerifyAllowed(req){
  const ip = String(req.headers['x-forwarded-for'] || req.socket.remoteAddress || '').split(',')[0].trim();
  const now = Date.now();
  const rec = billVerifyHits.get(ip) || { n:0, t:now };
  if(now - rec.t > 60000){ rec.n = 0; rec.t = now; }
  rec.n++;
  billVerifyHits.set(ip, rec);
  if(billVerifyHits.size > 5000) billVerifyHits.clear();
  return rec.n <= 40;
}
