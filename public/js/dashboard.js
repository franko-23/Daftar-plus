...
async function loadSubscriptionPlans() {
  ...
  try {
    ...
    subscriptionPlans = Array.isArray(data?.plans)
      ? data.plans
      : [];
...
async function loadCurrentSubscription() {
  try {
    const data =
      await api('/api/subscription/current');

    currentSubscription = data || null;

    const status = $('subStatus');
    const expiry = $('subExpiry');
    const card = $('subCurrentCard');
...
async function loadSubscriptionPayments() {
  ...
  try {
    ...
    const data =
      await api('/api/subscription/payments');
