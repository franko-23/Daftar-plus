(() => {
  ...
  async function loadSubscription(){
    ...
    if(plansRes){
      ...
      }catch(e){ setSubNotice('⚠️ Hitilafu ya plans: ' + e.message, 'badtxt'); }
    }
    if(subRes){
      try{ renderSubStatus(subRes); }
      catch(e){ setSubNotice('⚠️ Hitilafu ya status: ' + e.message, 'badtxt'); }
    }
    if(payRes){
      try{ renderSubPayments(payRes.payments || []); }
