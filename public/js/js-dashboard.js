(() => {
  ...
  async function loadSubscription(){
    const [plansRes, subRes, payRes] = await Promise.all([
      ...
      api('/api/subscription/payments')
    ]);
    if(!plansRes || !subRes || !payRes) return;
    const plans = plansRes.plans || [];
    subPlan = plans.find(p=>p.code==='business') || plans[0];
    if(subPlan) $('subPayBtn').textContent = `LIPA TSh ${money(subPlan.current_price_tzs)}`;
    renderSubStatus(subRes);
    renderSubPayments(payRes.payments || []);
